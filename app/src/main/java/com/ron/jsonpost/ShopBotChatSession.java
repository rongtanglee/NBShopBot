...
public class ShopBotChatSession{
    ...
    public String updateSimulatorState() throws IOException {
        if (userSimulatorID == null) {
            Log.d(TAG, "User simulator ID is not created!");
            throw new IOException("User Simulator ID is not created");
        }

        if (objSemanticFrame == null) {
            Log.d(TAG, "Semantic Frame is invalid or unavailable");
            throw new IOException("Semantic Frame is invalid or unavailable");
        }
